...
class Poisson2D:
    ...
    def l2_error(self, u):
        ...
        return np.sqrt(np.sum(self.h**2*(u - self.meshfunction(self.ue))**2))
    ...
    def eval(self, x, y):
        ...
        if j < self.N//2:
            ...
            jmax = min(self.N+1, j+2)
            jmin = jmax - 4
        return interpn((self.xij[imin:imax, 0], self.yij[0, jmin:jmax]), self.U[imin:imax, jmin:jmax], np.array([x, y]))
